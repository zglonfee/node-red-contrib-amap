module.exports = function (RED) {
    ...
    function AMapDistance(config) {
        ...
        this.on('input', function (msg, send, done) {
            // src
            var src_longitude = undefined;
            var src_latitude = undefined;
            if(config.src_type === 'static'){
                ...
                src_longitude = RED.util.evaluateNodeProperty(config.src_dyn_longitude, config.src_dyn_longitude_type, node, msg);
                src_latitude = RED.util.evaluateNodeProperty(config.src_dyn_latitude, config.src_dyn_latitude_type, node, msg);
            }
            var src_location_str = `${src_longitude},${src_latitude}`

            // dest
            var dest_longitude = undefined;
            var dest_latitude = undefined;
            if(config.dest_type === 'static'){
                ...
                dest_longitude = RED.util.evaluateNodeProperty(config.dest_dyn_longitude, config.dest_dyn_longitude_type, node, msg);
                dest_latitude = RED.util.evaluateNodeProperty(config.dest_dyn_latitude, config.dest_dyn_latitude_type, node, msg);
            }
